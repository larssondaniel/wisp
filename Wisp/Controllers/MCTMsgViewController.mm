...
- (void)viewDidLoad {
    ...
    _messageView.autoresizingMask =
        UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    [self.view addSubview:_messageView];

    if (![[NSUserDefaults standardUserDefaults]
            boolForKey:@"FetchFullMessageEnabled"]) {
        ...
        [op start:^(NSError *error, NSData *data) {
            ...
            MCOMessageParser *msg =
                [MCOMessageParser messageParserWithData:data];
            [_messageView setDelegate:self];
            [_messageView setFolder:_folder];
...
- (void)setMessage:(MCOIMAPMessage *)message {
    ...
    // Add seen flag
    [_session storeFlagsOperationWithFolder:@"INBOX"
                                       uids:[MCOIndexSet
                                                indexSetWithIndex:_message.uid]
...
- (MCOIMAPFetchContentOperation *)_fetchIMAPPartWithUniqueID:(NSString *)
                                  partUniqueID folder:(NSString *)folder {
    ...
    MCOIMAPFetchContentOperation *op = [_session
        fetchMessageAttachmentByUIDOperationWithFolder:folder
                                                   uid:[_message uid]
                                                partID:[part partID]
...
- (NSString *)MCOMessageView_templateForMessage:(MCOMessageView *)view {
    return @"<div style=\"padding-bottom: 20px; font-family: Helvetica; "
        @"font-size: 13px;\">{{HEADER}}</div><div>{{BODY}}</div>";
}
...
- (NSString *)MCOMessageView:(MCOMessageView *)view
                filteredHTML:(NSString *)html {
    return html;
}
...
- (NSData *)MCOMessageView:(MCOMessageView *)view
    ...
    if ([[NSUserDefaults standardUserDefaults]
            boolForKey:@"FetchFullMessageEnabled"]) {
        MCOAttachment *attachment = (MCOAttachment *)
            [[_messageView message] partForUniqueID:partUniqueID];
...
- (NSData *)_convertToJPEGData:(NSData *)data {
    ...
    [info setObject:(id)kCFBooleanTrue
             forKey:(__bridge id)kCGImageSourceCreateThumbnailWithTransform];
    [info setObject:(id)kCFBooleanTrue
             forKey:(__bridge id)kCGImageSourceCreateThumbnailFromImageAlways];
    [info setObject:(id)[NSNumber numberWithFloat : (float)IMAGE_PREVIEW_WIDTH]
             forKey:(__bridge id)kCGImageSourceThumbnailMaxPixelSize];
